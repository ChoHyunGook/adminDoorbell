...
export default function UserService(){
    ...
    const {
        access_jwt_secret, refresh_jwt_secret,ADMIN_ID,ADMIN_PASSWORD,
        LG_HELLO_ADMIN,LG_U_PLUS_ADMIN,SUNIL_ADMIN,RAMIAN_ADMIN
    } = applyDotenv(dotenv)
    ...
    return {
        companyLogin(req,res){
            const data = req.body
            const checkData = data.authId+":"+data.password
            if(checkData === LG_HELLO_ADMIN || checkData === LG_U_PLUS_ADMIN || checkData === SUNIL_ADMIN
                || checkData === RAMIAN_ADMIN || checkData === LG_HELLO_ADMIN
                || checkData === ADMIN_ID+":"+ADMIN_PASSWORD){
               ...
               res.status(200).send('Success')
                ...
                res.status(400).send('기업 아이디 또는 패스워드가 틀렸습니다.')
            }
        },
